...
object Scala_project {
  def main(args:Array[String]){
    ...
    val user_upload_dump = hqlContext.sql("select * from practical_exercise_1.user_upload_dump")
    user_upload_dump.show()

    val user = hqlContext.sql("select * from practical_exercise_1.user")
    user.show()

    val activitylog = hqlContext.sql("select * from practical_exercise_1.activitylog")
    activitylog.show()
    ...
    val result = hqlContext.sql("select * from userReportTemp")
    result.show()
  }
}
